    ...
    private void ostotapahtumaMenu() {
        ...
        while (valinta != 0) {
            ...
            if (valinta == 1) {
                System.out.println(verkkokauppa.listaaMyyjat());
                System.out.println();
                Myyja myyja = lueMerkkijono("Valitse näistä yksi");
                System.out.println(verkkokauppa.listaaTuotteet());
                System.out.println();
                Tuote tuote = lueMerkkijono("Valitse näistä yksi");
                System.out.println(verkkokauppa.listaaAsiakkaat());
                System.out.println();
                Asiakas asiakas = lueMerkkijono("Valitse näistä yksi");
                int maara = lueKokonaisluku(0, tuote.MAKSIMIHINTA, "Syötä myytävien tuotteiden kappalemäärä");
                double hinta = 0;
                if (asiakas instanceof KantaAsiakas) {
                    hinta = maara * tuote.getHinta() * ((100-kantaasiakas.getAlennusprosentti()) * 0.01);
                } else {
                    hinta = maara * tuote.getHinta() * ((100 - asiakas.getAlennusprosentti()) * 0.01);
                }

                Ostotapahtuma osto = Ostotapahtuma(asiakas, myyja, tuote, maara);
                lisaaTapahtuma(osto);
            ...
            if (valinta == 2) {
                for (Ostotapahtuma ot : tapahtumat) {
                    System.out.println("1. " + ot);
                    int luku = lueKokonaisluku(0, tapahtumat.size(), "Anna poistettavan tapahtuman järjestysluku");
                    tapahtumat.remove(luku - 1);
                }
            }
        }
    }
